...
def send():
    url_api = 'http://78.109.28.77/fields/api/v01/train_weight'
    storage = 'Фастів ХПП'
    data = wagons()

    try:
        response = requests.post(url_api, json={'storage': storage, 'wagons': data})
...
def wagons():
    ...
    # упаковка в список словарей для json
    wagons = []
    for each in request:
        # print(each[0])
        wagons.append({
    ...
    if wagons:
        with open('last_id_vagon', 'w') as file:
            file.write(str(wagons[-1]['id_vagon']))
